...
const { smsg, start, success} = require('./basexd/lib/myfunc')
...
const xd = makeWASocket({ logger: pino ({ level: 'silent' }), printQRInTerminal: true, auth: state, browser: ["SanXd", "Safari", "1.0.0"]})
...
m = smsg(xd, m, store)
require('./sanzy')(xd, m, chatUpdate, store)
} catch (err) {
console.log(err)}})
